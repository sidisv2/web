    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-white mt-1">Planes y Métodos de Pago para Tu Agencia</h1>
          <p className="text-xs text-slate-400 mt-0.5">
            Selecciona tu plan, elige la moneda de tu país y utiliza cualquiera de nuestras pasarelas integradas (Mercado Pago, Tarjeta, PayPal, SPEI/PSE o Crypto).
          </p>
        </div>

        {/* Currency Switcher Pill */}
        <div className="bg-black/40 border border-white/10 p-1 rounded-xl flex items-center gap-1 self-start md:self-auto">
          {CURRENCIES.map((c) => (
            <button
              key={c.code}
          ...
          return (
            <div
              ...
              className={`p-6 rounded-2xl border transition-all cursor-pointer flex flex-col justify-between relative ${
                isSelected
                  ? 'bg-white/[0.04] border-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.15)] ring-1 ring-emerald-500/50'
                  : 'bg-black/30 border-white/5 hover:border-white/20'
              }`}
            >
              {plan.badge && (
                <div className="absolute -top-3 right-4 px-3 py-0.5 rounded-full bg-gradient-to-r from-emerald-500 to-emerald-600 text-slate-950 font-black text-[10px] tracking-wider uppercase shadow-md">
                  {plan.badge}
                </div>
              )}
              ...
              <button
                ...
                className={`w-full mt-6 py-2.5 px-4 rounded-xl font-bold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer ${
                  isSelected
                    ? 'bg-emerald-500 text-slate-950 hover:bg-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.3)]'
                    : 'bg-white/10 text-white hover:bg-white/20'
                }`}
              >
                <span>{isSelected ? 'Proceder al Pago' : 'Seleccionar Plan'}</span>
      ...
      <div className="bg-black/30 border border-white/5 rounded-2xl p-6 md:p-8 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
          <div>
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
              <Wallet className="w-5 h-5 text-emerald-400" />
              Métodos de Pago Disponibles en Latinoamérica
            </h3>
            <p className="text-xs text-slate-400 mt-0.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <button
            ...
            className={`p-4 rounded-xl border text-left transition-all space-y-3 cursor-pointer ${
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">USDT / USDC (Red TRC20)</p>
            </div>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6 space-y-4">
        ...
        <p className="text-xs text-slate-400">
          Puedes pegar aquí tus propios enlaces de pago (Stripe Checkout, Mercado Pago Link, PayPal Me o enlace de WhatsApp) para que tus clientes o equipo se dirijan a tu pasarela directa:
        </p>
      ...
      {showCheckoutModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-white/10 rounded-2xl max-w-lg w-full p-6 space-y-6 relative overflow-hidden shadow-2xl">
            
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-white/5 pb-4">
              <div>
                <span className="text-[10px] uppercase font-bold text-emerald-400 tracking-wider">
                  Checkout - {activePlan.name}
                </span>
            ...
            {paymentCompleted ? (
              <div className="py-8 text-center space-y-3">
                ...
                <p className="text-xs text-slate-300 max-w-xs mx-auto">
                  Tu plan <strong className="text-emerald-400">{activePlan.name}</strong> y tu agente <strong>Aria Prop</strong> están listos para atender a tus clientes en LATAM.
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5 space-y-3 text-xs">
                  ...
                  {paymentMethod === 'transfer' && (
                    <div className="p-3 bg-black/50 rounded-lg space-y-1 font-mono text-[11px] text-slate-300 border border-white/5">
                  ...
                  {paymentMethod === 'crypto' && (
                    <div className="p-3 bg-black/50 rounded-lg space-y-1 font-mono text-[11px] text-slate-300 border border-white/5 flex items-center justify-between">
                      <div>
